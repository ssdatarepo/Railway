...
- Groups rows by CIN to guarantee the newest document is attempted first.
- Truncates text to 30,000 characters to prevent Google Sheets API crashes.
- Includes exponential backoff for Google API rate limits and 50x errors.
- Uses space-agnostic parsing to handle corrupted PDFs missing whitespace.
"""
...
# Limit to 30,000 characters per cell to prevent API errors
...
# Modified to not require a space after the dot/parenthesis
NUMERIC_ITEM_PATTERN = re.compile(r"^(\d{1,2})[\.\)]")
...
def extract_main_objects_from_pdf_bytes(pdf_bytes: bytes) -> tuple[str, str | None]:
    ...
    for i, line in enumerate(lines):
        u = line.upper()
        u_clean = u.replace(" ", "")  # Space-agnostic for corrupted PDFs

        if start_idx is None:
            if "REGISTEREDOFFICEOFTHECOMPANY" in u_clean:
                start_idx = i + 1
            continue
    ...
    if start_idx is not None and stop_idx is not None and stop_idx > start_idx:
        ...
        for line in lines[start_idx:stop_idx]:
            ...
            if PAGE_FOOTER_PATTERN.match(u):
                continue
            if "OBJECTSTOBEPURSUEDBYTHECOMPANY" in u_clean:
                continue
            out.append(line)
    ...
    for line in lines:
        ...
        if not capture:
            # Check for generic (A) and OBJECT even if missing spaces
            if "(A)" in u_clean and "OBJECT" in u_clean:
                capture = True
            continue
        ...
        # Space-agnostic fallback stop markers
        if "LIABILITYOFTHEMEMBER" in u_clean or "SUBSCRIBERDETAILS" in u_clean or "SIGNEDBEFOREME" in u_clean:
            break
    ...
    result = " ".join(out).strip()
    if result:
        # Final sanity check - do we have any objects?
        if "OBJECT" not in result.upper().replace(" ", ""):
            return result, "Warning: 'object' not found in extracted text"
        return result, None
    ...
    for row, link in rows_list:
        ...
        try:
            ...
            if warning:
                ...
                status_value = warning if warning.startswith("Warning") else f"Warning: {warning}"
            else:
                if extracted:
...
def get_worksheet_with_retry(gc, spreadsheet_id, sheet_name, max_retries=5):
    """Fetches the Google Sheet with exponential backoff for 50x errors and rate limits."""
    for attempt in range(max_retries):
        try:
            return gc.open_by_key(spreadsheet_id).worksheet(sheet_name)
        except gspread.exceptions.APIError as e:
            # 429 = Too Many Requests, 50x = Server Errors
            if getattr(e.response, 'status_code', None) in [429, 500, 502, 503, 504]:
                if attempt < max_retries - 1:
                    sleep_time = 2 ** attempt  # 1s, 2s, 4s, 8s
                    print(f"Google API Error {e.response.status_code}. Retrying in {sleep_time}s...")
...
def main():
    ...
    ws = get_worksheet_with_retry(gc, INPUT_SPREADSHEET_ID, INPUT_SHEET_NAME)
    ...
    for chunk_start in range(0, len(rows_to_process), BATCH_SIZE):
        ...
        for sub_start in range(0, len(chunk), WRITE_BATCH_SIZE):
            ...
            for row, cin, link in sub_chunk:
                ...
                # --- TRUNCATE TEXT IF OVER MAX CHARACTERS ---
                if len(cell_value) > MAX_CELL_CHARS:
                    cell_value = cell_value[:MAX_CELL_CHARS] + f"\n\n...[TRUNCATED DUE TO {MAX_CELL_CHARS} CHAR LIMIT]"
            ...
            # Attempt batch update with retry logic
            for attempt in range(5):
                try:
                    ...
                    if getattr(e.response, 'status_code', None) in [429, 500, 502, 503, 504] and attempt < 4:
                        sleep_time = 2 ** attempt
                        print(f"Write API Error {e.response.status_code}. Retrying in {sleep_time}s...")
                        time.sleep(sleep_time)
                    else:
